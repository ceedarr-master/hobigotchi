...
export const ButtonXL = ({ children, onClick, theme = "sys", disabled }) => (
  <button 
    onClick={onClick}
    disabled={disabled}
    className={`${baseStyle} w-full h-20 text-hobi-xl font-bold justify-center border-[4px] rounded-[20px] 
...
export const ButtonL = ({ label, icon, onClick }) => (
  <button 
    ...
    w-full h-20 px-5 justify-between border-[4px] border-hobi-black rounded-[20px] bg-white shadow-[0_8px_0_0_#1b1f4b] text-hobi-lg 
    hover:-translate-y-[2px] active:translate-y-[4px] active:shadow-none"
  >
...
export const ButtonM = ({ label, icon, theme, onClick, isActive }) => (
  <button 
    onClick={onClick}
    className={`${baseStyle} w-20 h-20 flex-col gap-1 justify-center border-[4px] rounded-[20px]
    ${theme.border} 
    ${isActive 
